...
const o = {
    ...
    duplicate: { value(n) { // n = 1 (returns [ Node ]) is NOT the same as n = null | undefined (returns Node)
        if (this instanceof Document)
            throw new TypeError("duplicate: Cannot duplicate a Document.");
        ...
        if (typeof n !== "number" || n < 1)
            throw new TypeError("duplicate: Argument must be a positive integer.");
    ...
    create: { value(tagName, attributes = {}, location = {}) {
        if (typeof tagName !== "string")
            throw new TypeError("create: tagName must be a string.");
            ...
            switch (k) {
                ...
                case "style": {
                    if (typeof v !== "object" || v === null)
                        throw new TypeError("create: style must be an object.");
        ...
        if (location?.start === true)
            this.insertBefore(el, this.firstChild);
        else if (location?.end === true)
            this.appendChild(el);
        else if (location?.before instanceof Node) {
            if (location.before.parentNode)
                location.before.parentNode.insertBefore(el, location.before);
            else
                throw new Error("create: before location is not part of the document.");
        } else if (location?.after instanceof Node) {
            if (location.after.parentNode)
                location.after.parentNode.insertBefore(el, location.after.nextSibling);
            else
                throw new Error("create: after location is not part of the document.");
        }
